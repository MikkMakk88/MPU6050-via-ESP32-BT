...
FRAME_RATE_CAP = 30
SMOOTH_MOD = 1				# set to 1 to leave angles unmodified
ANGLE_MULTIPLIER = 1		# not currently used
...
def smooth_angles(angles):
	global SMOOTH_MOD
	return [divmod(a, SMOOTH_MOD)[0] * SMOOTH_MOD for a in angles]
...
def update_rotation():
	...
	angles = [(a * 360 / 2**16) - ANGLES_OFFSET[i] for i, a in enumerate(get_data())]
...
def main():
	...
	# initial calibration
	input("Make sure the module is facing up, then hit Enter.")
	ANGLES_OFFSET = [a * 360 / 2**16 for a in get_data()]
